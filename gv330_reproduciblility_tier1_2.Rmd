...
data$Happiness_score[1] == scaled_data$Happiness_score[1] # should be FALSE -- we could code up a warning here if we want, checks that the first value of happiness in data is same as first in scaled... if they are the same then FALSE
...
```{r analysis}
# we could call the cleaned data back into R at this point if we want (or not) to check that the processed data set is clean and there are no errors:
...
plot <- ggplot(scaled_data) +
          ...
          theme_minimal()
ggsave(plot, file = paste0(figure_dir,"plot_gdp_pc.jpg")) #ggsvae will save the graph with the dimensions of the bottom right box so should specify the dimensions to be properly reproducible
