...
# now turn out simple operation into a function... pure function = for same inputs and always return correct and same output... inputs do not need to be objects... x and y are 'locally scoped objects' and so line of code defines x and y as within the brackets of the function... if wither x and y are not locally defined it will jummp up a level and call on the x or y that are defined as objevcts in main environment... must be careful about scoping of functions
add_up <- function(x, y){
  x + y
}
...
# our function to scale the data -- the code changes slightly here to allow variables to be passed through... 2 argumetns: tell what data file and tell what arguments want to scale
